...
//         // optional: purge older-than-30-days on load
...
const Trash = () => {
  ...
  const restoreMessage = async (id) => {
    const ok = window.confirm("Restore this message back to All Messages?");
    if (!ok) return;
    try {
                ...
                filtered.map((m) => {
                  ...
                  return (
                    <tr key={m._id} className="border-b">
                      ...
                      <td className="py-3 pl-2 pr-0 text-right space-x-1">
                        <button
                          className="inline-flex items-center justify-center p-2 rounded hover:bg-gray-50 text-gray-700"
                          title="Restore"
